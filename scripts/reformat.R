...
raw_mtx <- as(t(readMM(paste0('../data/', sample_id, '_kb_out/counts_unfiltered/cells_x_genes.mtx'))), 'CsparseMatrix') # load mtx and transpos$
rownames(raw_mtx) <- read.csv(paste0('../data/', sample_id, '_kb_out/counts_unfiltered/cells_x_genes.genes.txt'), sep = '\t', header = F)[,1] #$
